...
public class TimedOutByteWriting {

    private final DeviceConnection byteWriter;
    ...
    TimedOutByteWriting(DeviceConnection byteWriter, byte oneByte, long timeout) {
        if (timeout <= 0) {
    ...
    public TimedOutByteWriting(DeviceConnection byteWriter, byte[] bytes, long timeout) {
        if (timeout <= 0) {
            throw new IllegalArgumentException("Invalid time out value : "+timeout+". Must be greater than 0");
        }
        this.byteWriter = Objects.requireNonNull(byteWriter, "byteWriter");
    ...
    private class WriteRunnable extends TimedOutByteWritingRunnable {
        ...
        public void run() {
            ...
            try {
                if (onlyOneByte) {
                    byteWriter.write(new byte[]{oneByte});
                } else {
